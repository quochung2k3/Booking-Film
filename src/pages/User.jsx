...
function User({ onLogout }) {
    ...
    if (token) {
      try {
        const response = await axios.get(
          `http://localhost:3000/api/v1/bookings/${movie.id}`,
          {
  ...
  return (
    <Container>
      ...
      <TabsContainer>
        <Tab
          active={activeTab === "PHIM SẮP CHIẾU"}
          onClick={() => handleTabClick("PHIM SẮP CHIẾU")}
        >
          PHIM SẮP CHIẾU
        </Tab>
        <Tab
          active={activeTab === "PHIM ĐANG CHIẾU"}
          onClick={() => handleTabClick("PHIM ĐANG CHIẾU")}
        >
          PHIM ĐANG CHIẾU
        </Tab>
        <Tab
          active={activeTab === "SUẤT CHIẾU ĐẶC BIỆT"}
